...
router.post("/", validateReview, isLoggedIn, wrapAsync( async(req,res) =>{
    let listing = await Earth.findById(req.params.id)
    ...
    res.redirect(`/earths/${listing._id}`)
 }))
 ...
 router.delete(
    "/:reviewId",isLoggedIn,isReviewAuthor, wrapAsync( async(req, res) =>{
        ...
        await Earth.findByIdAndUpdate(id, {$pull:{reviews: reviewId}})
        await Review.findByIdAndDelete(reviewId)
        req.flash("success", "Your Review's are Deleted!")
        res.redirect(`/earths/${id}`)
    })
 )
